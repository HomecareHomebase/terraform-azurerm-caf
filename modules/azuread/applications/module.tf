resource "azuread_application" "app" {
  ...
  display_name = var.global_settings.passthrough ? format("%s", var.settings.application_name) : join("_", compact(concat(var.global_settings.prefixes == null ? [] : var.global_settings.prefixes, [var.settings.application_name], var.global_settings.suffixes == null ? [] : var.global_settings.suffixes)))
  owners = [
