locals {
  ...
  combined_objects_networking                        = merge(tomap({ (local.client_config.landingzone_key) = merge(module.networking, module.networking_datas) }), try(var.remote_objects.vnets, {}))
  ...
  combined_objects_resource_groups                   = merge(tomap({ (local.client_config.landingzone_key) = merge(module.resource_groups, module.resource_group_datas) }), try(var.remote_objects.resource_groups, {}))
